  ...
  RSStandard{..} -> do
    let
      ...
      prefHeader =
        prefAppliedHeader $
          Preferences
            (if null pkCols && isNothing (qsOnConflict iQueryParams) then Nothing else preferResolution)
            preferRepresentation
            preferCount
            preferTransaction
            preferMissing
    ...
    let isInsertIfGTZero i =
          if i <= 0 && preferResolution == Just MergeDuplicates
            then
              HTTP.status200
            else
              HTTP.status201
...
actionResponse (DbCallResult CallReadPlan{crMedia, crInvMthd = invMethod, crProc = proc} resultSet) ctxApiRequest@ApiRequest{iPreferences = Preferences{..}, ..} _ _ _ _ _ = case resultSet of
  RSStandard{..} -> do
    ...
    let (status', headers', body) =
          if Routine.funcReturnsVoid proc
            then
...
respondInfo allowHeader =
  let allOrigins = ("Access-Control-Allow-Origin", "*")
   in Right $ PgrstResponse HTTP.status200 [allOrigins, (HTTP.hAllow, allowHeader)] mempty

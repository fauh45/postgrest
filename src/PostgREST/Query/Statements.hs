...
prepareWrite selectQuery mutateQuery isInsert isPut mt handler rep resolution pKeys prepared =
  (result, sql, trackedParams finalQuery)
 where
  ...
  querySnippet =
    rawSQL "WITH "
      ...
      <> locF
      <> rawSQL " AS header, "
      <> handlerF Nothing handler
      <> rawSQL " AS body, "
      <> responseHeadersF
      <> rawSQL " AS response_headers, "
      <> responseStatusF
      <> rawSQL " AS response_status, "
      <> pgrstInsertedF
      <> rawSQL " AS response_inserted "
      <> rawSQL "FROM ("
      <> selectF
      <> rawSQL ") _postgrest_t"
  ...
  querySnippet =
    rawSQL "WITH "
      ...
      <> ( if funcReturnsSingle rout
            then rawSQL "1"
            else rawSQL "pg_catalog.count(_postgrest_t)"
         )
      <> rawSQL " AS page_total, "
      <> handlerF (Just rout) handler
      <> rawSQL " AS body, "
      <> responseHeadersF
      <> rawSQL " AS response_headers, "
      <> responseStatusF
...
mtSnippet mediaType sqlSnippet = case mediaType of
  MTVndPlan _ fmt opts ->
    rawSQL "EXPLAIN ("
      <> rawSQL (BS.intercalate ", " (fmtPlanFmt fmt : (fmtPlanOpt <$> opts)))
      <> rawSQL ") "
      <> sqlSnippet
  _ -> sqlSnippet
 where

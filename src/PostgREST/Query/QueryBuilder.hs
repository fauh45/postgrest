...
readPlanToQuery node@(Node ReadPlan{select, from = mainQi, fromAlias, where_ = logicForest, order, range_ = readRange, relToParent, relJoinConds, relSelect, relSpread} forest) =
  rawSQL "SELECT "
    ...
    <> ( if null logicForest && null relJoinConds
          then mempty
          else rawSQL " WHERE " <> intercalateSnippet " AND " (map (pgFmtLogicTree qi) logicForest ++ map pgFmtJoinCondition relJoinConds)
       )
    <> rawSQL " "
...
mutatePlanToQuery (Insert mainQi iCols body onConflict putConditions returnings _ applyDefaults) =
  rawSQL "INSERT INTO "
    <> fromQi mainQi
    <> (if null iCols then rawSQL " " else rawSQL "(" <> cols <> rawSQL ") ")
...
callPlanToQuery (FunctionCall qi params arguments returnsScalar returnsSetOfScalar returnsCompositeAlias returnings) pgVer =
  rawSQL "SELECT "
    <> (if returnsScalar || returnsSetOfScalar then rawSQL "pgrst_call.pgrst_scalar" else returnedColumns)
    <> rawSQL " "
    <> fromCall
 where
  ...
  fromCall = case params of
    ...
    KeyParams prms -> case arguments of
      ...
      JsonArgs json ->
        fromJsonBodyF json ((\p -> CoercibleField (ppName p) mempty False Nothing (ppTypeMaxLength p) Nothing Nothing False) <$> prms) False True False
          <> rawSQL ", "
          <> rawSQL "LATERAL "
  ...
  callIt argument
    | pgVer < pgVersion130 && returnsCompositeAlias = rawSQL "(SELECT (" <> fromQi qi <> rawSQL "(" <> argument <> rawSQL ")).*) pgrst_call"
